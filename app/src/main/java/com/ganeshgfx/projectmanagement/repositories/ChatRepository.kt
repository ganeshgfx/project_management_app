...
private const val TOKEN_SIZE = 50
...
//            "{Title:${project.title} , Description:${project.description},owner : ${owner.displayName}}"

        val prompt = "Pretend that you are a assistant you know following information, give short replies,If there too much information reduce it to only most relevant information, Only replies based on provided information. project information : $projectInfo $msg"
    ...
    suspend fun send(prompt: String): Chat? {
        ...
        if (response.isSuccessful) {
            log(response.raw())
            response.body()?.choices?.get(0)?.text?.let { chat = Chat(it.trim(), false) }
